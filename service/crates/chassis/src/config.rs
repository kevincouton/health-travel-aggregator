...
//! ./data/<platform>.db.
//!
//! This file is a template: `health-travel-aggregator` is a valid Rust string
//! literal, so the crate compiles as-is with the platform name literally
//! "health-travel-aggregator" until bin/instantiate-platform substitutes it — the
//! same behavior as the Go template.

use std::env;

/// Platform name, substituted per clone by bin/instantiate-platform.
pub const PLATFORM_NAME: &str = "health-travel-aggregator";
